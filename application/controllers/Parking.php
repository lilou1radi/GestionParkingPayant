...
class Parking extends Admin_Controller 
{
	...
	public function index()
	{
		...
		foreach ($parking_data as $k => $v) {
			$result[$k]['parking'] = $v;
			$category_data = $this->model_category->getCategoryData($v['vehicule_cat_id']);
	...
	public function create()
	{
        ...
        if ($this->form_validation->run() == TRUE) {
        	...
        	$data = array(
        		...
        		'vehicule_cat_id' => $this->input->post('vehicle_cat'),
	...
	public function edit($id = null)
	{
		...
		if($id) {
            ...
            // true case
	        	...
	        	$get_used_rate = $this->model_rates->getCategoryRate($save_parking_data['vehicule_cat_id']);
	...
	public function printInvoice($id)
	{
		...
		if($id) {
			...
			$vehicle_category = $this->model_category->getCategoryData($parking_data['vehicule_cat_id']);

			$rate_data = $this->model_rates->getRateData(1);
				...
				<body>
					<div class="main-content">
						<div class="company-info">
							...
							<div class="parking-slip"><h2>PAID PARKING</h2></div>
						</div>
						<div class="parking-info">
							<table class="table">
								...
								<tr>
									<td>Type de Vehicule: '.ucwords($vehicle_category['name']).' </td>
								</tr>
								<tr>
									<td>Num Parking: '.$parking_data['parking_code'].' </td>
								</tr>
								<tr>
									<td>Matricule: '.$rate_data['matricule'].'  </td>
								</tr>
							</table>
	...
	public function getCategoryRate($id) 
	{
		if($id) {
			...
			foreach ($rate_data as $k => $v) {
				$html .= '<option value="'.$v['id'].'">'.$v['matricule'].'</option>';
			}
